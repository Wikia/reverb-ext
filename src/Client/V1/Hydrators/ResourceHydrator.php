...
class ResourceHydrator implements Hydrator {
	...
	public function hydrate( Document $document ) {
		...
		$primary = $document->primaryResources()->map( static function ( $primary ) use ( $resources ) {
	...
	protected function hydrateRelations( ResourceObject $object, Collection $resources ): array {
		return $object->relations()->map( static function ( $relations, $relationship ) {
			return ( new Collection( $relations ) )->map( static function ( $relation ) use ( $relationship ) {
				return $relation + [
		...
		->flatten( 1 )
		->mapToGroups( static function ( $relation ) use ( $resources ) {
